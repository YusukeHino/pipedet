...
from tools.cropper import LargeImage
...
if __name__ == '__main__':
    ...
    input_dir_list = [i for i in os.listdir(INPUT_DATA_ROOT)]
    for input_dir in input_dir_list:
        if input_dir.endswith("train"):
            ...
            tag = "VALIDATE"
            output_dir = "validate"
        elif input_dir.endswith("eval"):
            tag = "TEST"
            output_dir = "test"
        else:
            continue
        full_path_input = INPUT_DATA_ROOT + "/" + input_dir
        full_path_output = OUTPUT_DATA_ROOT + "/" + output_dir
        try:
            os.makedirs(full_path_output)
        except FileExistsError:
            pass
        for xml_file in glob.glob(full_path_input + '/*.xml'):
            tree = ET.parse(xml_file)
            root = tree.getroot()
            image_name = root.find('filename').text
            full_path_input_image = full_path_input + "/" + image_name
            bboxes = []
            for member in root.findall('object'):
                bbox = [0] * 4
